...
a <- Sys.time()
combined_mice <- mice(combined, m = 5, seed = 2022, method = "cart")
b = Sys.time()
print(b-a)
...
a <- Sys.time()
combined_mice <- parlmice(combined, m = 18, seed = 2022, method = "cart", n.core = 7, n.imp.core = 2, cluster.seed = 1995)
b = Sys.time()
print(b-a)
...
log_reg = train(model_recipe, data= data_train, method = model_type, trControl = trControl, tuneLength = 10)
summary(data_train)
...
for (i in colnames(combined)) {
  if (is.factor(combined[[i]])) {
    combined[[i]] <- replace_na(combined[[i]], "missing")
  }
}

combined$CBQ596 = as.factor(combined$CBQ596)
combined_new <- combined %>% mutate_if(is.character, as.factor)
combined_new <- combined_new %>% mutate_if(is.factor, make.names)

data_index <- createDataPartition(combined_new[["CBQ596"]], p = 0.7, list = FALSE)
data_train <- combined_new %>% slice(data_index)
data_test <- combined_new %>% slice(-data_index)
...
a <- Sys.time()
rf_combined <- train(model_recipe, data = data_train, method = model_type, trControl = trControl, metric = 'ROC')
b <- Sys.time()
